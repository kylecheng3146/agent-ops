...
async function readPyproject(filePath: string): Promise<MetadataReadResult> {
  let status;
  try {
...
function hasExplicitPytestEvidence(source: string): boolean {
  return source.split(/\r?\n/u).some((line) => (
    /^\s*\[tool\.pytest\.ini_options\]\s*(?:#.*)?$/u.test(line)
  ));
}
...
function pytestProposal(evidence: DiscoveryEvidence[]): VerifierProposal {
  return {
    id: "python:pytest",
    command: "python",
    args: ["-m", "pytest"],
...
export async function discoverPythonProject(
  ...
  if (hasMalformedTableHeader(metadata.source)) {
    return userDecision(
      "invalid-manifest",
      "pyproject.toml contains malformed table metadata.",
      evidence
    );
  }
  if (!hasExplicitPytestEvidence(metadata.source)) {
    return userDecision(
      "no-known-commands",
      "No explicit pytest configuration was found.",
      evidence
    );
  }
...
export const pythonDiscoveryAdapter: DiscoveryAdapter = {
  id: "python",
