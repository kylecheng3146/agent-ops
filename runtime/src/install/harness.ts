...
export function managedRules(
  ...
  if (context.capabilities.includes("rules")) {
    lines.push(
      ...
      "5. Obtain complete independent review before claiming completion, via",
      "   `agent-ops review --task <task-id> --yes` (or the CLI's equivalent invocation).",
      "   Never call a",
      ...
      "   permission prompts.",
      "   Start this command through the trusted outer host runner as its first",
      "   and only invocation. On Codex, the outer tool request must use",
      "   `sandbox_permissions: \"require_escalated\"` so network and loopback",
      "   are available; in that same request, unset",
      ...
      "   set `AGENT_OPS_HOST` to the current CLI id so it is excluded; with two",
      "   targets both run in configured order; with one target it runs twice.",
      "   The first session is necessary; a PASS then requires an adversarial",
      "   re-check carrying the first report. A first FAIL or NOT_RUN stops.",
      "",
  ...
  if (context.capabilities.includes("task")) {
    lines.push(
      ...
      "`agent-ops task create` from the main checkout before your first edit:",
      "it creates this session's worktree and puts the task there (`agent-ops",
      "worktree add <name> --session <session-id>` does the same without a",
      "task). Work only inside the printed path",
      "(Claude Code: EnterWorktree with that path; otherwise use it as every",
      "command's working directory). Commit, then run verify and review for",
      "each task with the printed `--base`; do not run `task complete` in the",
      "worktree. Leave it (Claude Code: ExitWorktree with action keep) and run",
      "`agent-ops worktree finish <name>` from the main checkout: it completes",
      "every task in the worktree, subtasks first, then merges. A finish that",
      "reports an incomplete task names it; verify or review that task again",
      "in the worktree and finish again. A finish that reports a conflict is",
